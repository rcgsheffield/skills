...
Use [Prettier](https://prettier.io/) to auto-format Markdown files before submitting:

```bash
npx prettier --write "**/*.md"
```

Prettier is available via `npx` without installation. Review diffs after running — it normalises heading spacing, list indentation, code fences, and trailing newlines but does not change content.
